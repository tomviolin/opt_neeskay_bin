#!/usr/bin/php
<?
# this is for the bathyfast table
	...
	$result = mysql_query("select max(recdate) from bathyfast");
	...
	echo "highest date in the bathyfast table is ".$maxbathy[0]."\n";
	if ($maxbathy[0] == "") {
		$maxbathy[0] = "2007-01-01";
	}

	// start from highest date in bathy
	$start = date("Y-m-d H:i:s",strtotime($maxbathy[0])-60*60);
	echo "Cleaning from ".$start." onwards.\n";
	$query = "select recdate, gpslat, gpslng, depthm from trackingdata_flex where recdate > '".$start."' and recdate < '2015-01-01' order by recdate";
	...
	while (($row = mysql_fetch_array($result)) != NULL) {
		...
		if ($avg != NULL) {
			if (abs($depth - $avg) < 4) {
				...
				$query2 = ("insert ignore into bathyfast (recdate,lat,lng,depth) "
					."values ('{$row['recdate']}','".($row['gpslat']*100000)."','".($row['gpslng']*100000)."','".($depth*100000)."')");
